...
def file_open(filepath: str, mode: str):
    ...
    if not isinstance(mode, str):
        raise TypeError("File open mode must be string")

    return {
        "file": open(filepath, mode),
        "name": filepath.rstrip("/").rsplit("/", 1)[-1],
    }
...
def encode_data(data):
    ...
    if isinstance(data, [int, float, list, tuple, dict, bool]):
        ...
        raise TypeError(
            "Data to encode must be of int, float, list, tiple, dict, bool, str, or bytes"
        )
...
def send_file(file_dict, filename: str = None):
    ...
    if not isinstance(file_dict, dict):
        try:
            ...
            raise TypeError(
                "Please use the file_open function as the standard open function will not return the filename as metadata"
            )
    else:
        filename = file_dict["name"]
...
def receive(json_data: dict):
    ...
    if not valid:
        raise DataCorruptionError(
            "Invalid Data: Data Hashes do not match! Possibly corrupted / tampered data?"
        )
